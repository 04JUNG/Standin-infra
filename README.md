...
값이 JSON이면 사람별 토큰으로 읽힌다 — 아래 「운영 대시보드 → 토큰은 사람마다 다르게 준다」 참고.
...
이 시크릿은 값이 **JSON이면 사람별 토큰으로 읽힌다**(BFF가 가른다).

```json
{ "보라": "48자-무작위", "도원": "48자-무작위", "동원": "48자-무작위" }
```

키가 곧 `admin_access_audit.reviewer`에 남는 이름이고, 대시보드 우측 상단에도 그 이름이 뜬다.
값이 JSON이 아니면 예전처럼 단일 토큰(`beta-reviewer`)으로 동작하므로 전환에 다운타임이 없다.

**토큰 하나를 여럿이 나눠 쓰지 않는다.** 이 화면은 사용자가 올린 실제 사진과 그 결과를 보여
준다. 토큰이 하나면 감사 로그에 전원이 같은 이름으로 남아 누가 누구의 사진을 열었는지 복원할
수 없고, 한 명이 빠질 때 전원의 토큰을 갈아야 한다.

같은 이유로 **팀원에게 이 시크릿 읽기 IAM을 주지 않는다** — 읽을 수 있으면 서로의 토큰까지
보인다. 한 사람이 쥐고 각자에게 자기 것만 1:1로 건넨다.

토큰에는 **만료가 없다.** 자동 회전을 걸어 두지 않았고 `generateSecretString`은 생성 시점에만
적용되므로 `cdk deploy`로는 값이 바뀌지 않는다. 값을 바꾼 뒤에는 **BFF를 강제 재배포해야**
새 태스크가 읽는다. 발급·전달·회전 절차는 `Standin-app-server/README.md`의
「팀원에게 열어 줄 때는 사람별 토큰으로 준다」 참고.
